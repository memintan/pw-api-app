...
test.beforeEach(async ({page}) => {
  ...
  await page.getByRole('textbox', {name: "Email"}).fill('mehmetetan@gmail.com')
...
test('delete article', async({page, request}) => {
  const response = await request.post('https://conduit-api.bondaracademy.com/api/users/login', {
    data: {
      "user":{"email":"mehmetetan@gmail.com","password":"Abc123"}
    }
  })
  const responseBody = await response.json()
...
test('create article', async({page, request}) => {
  ...
  await page.getByRole('textbox', {name: 'Write your article (in markdown)'}).fill('We like to use olaywright for automation')
